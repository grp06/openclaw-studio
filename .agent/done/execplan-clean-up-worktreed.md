...
- [ ] Add server-side cleanup preview + deletion flow with strict validation and warnings.
- [ ] Add client API helpers + UI flow to preview and trigger cleanup.
- [ ] Add tests and verification steps for cleanup selection and client calls.
...
3. Verification: Run `npm test -- --runInBand tests/unit/projectsCleanup.test.ts tests/unit/projectsStore.test.ts` and confirm all tests pass.
...
3. Verification: Run `npm test -- --runInBand tests/unit/projectsClient.test.ts` and confirm it passes.
